...
시간/겹침 계산, 입력 검증, 공유·세션 토큰, 비용 분배, 이벤트 검증 등 회귀 위험이 큰 순수 로직을 단위 테스트로 보호합니다.

## 보안

- 스케줄 접근은 로그인·공유 접근 시 발급되는 **서명된 HttpOnly 세션 쿠키**로 인가합니다. 각 `[id]` 엔드포인트는 요청자의 소유권을 검증합니다.
- 공유 링크는 HMAC 서명 + 만료가 포함된 토큰을 사용합니다.
- 입력값(시간·겹침·비밀번호 정책)은 클라이언트와 서버 양쪽에서 검증합니다.
